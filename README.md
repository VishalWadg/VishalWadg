# Hi, I'm Vishal Wadgaonkar 👋

🎓 Computer Engineering Graduate (2025) from Pune, Maharashtra  
💻 Full Stack Developer — Java · Spring Boot · React.js · TypeScript  
🔭 Currently building real-world projects  
📫 Reach me: wadgaonkarvishal54@gmail.com  

---

## 🛠️ Tech Stack

**Languages:** Java (OOP) · JavaScript (ES6+) · TypeScript · C++  
**Frontend:** React.js · Redux · HTML5 · CSS3  
**Backend:** Spring Boot · REST APIs  
**Database:** MySQL  
**Tools:** Git · GitHub · Jupyter Notebook  

---

## 🚀 Projects

### 🔐 PhishGuard — Phishing Detection System
> Detects phishing threats across social media: messages, URLs, and attachments

- Built ML models to classify and flag malicious content
- Integrated third-party security APIs for real-time threat intelligence
- Tech: Python · scikit-learn · REST APIs · Java
- [View Repository →](https://github.com/VishalWadg/PhishGuard)

---

### 🌐 Aniverse — Anime & Manga Community App
> A community platform for anime and manga enthusiasts

- Built with TypeScript for type-safe, scalable frontend code
- [View Repository →](https://github.com/VishalWadg/Aniverse)

---

## 🤝 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat&logo=linkedin)](https://linkedin.com/in/vishalwadgaonkar)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=flat&logo=gmail)](mailto:wadgaonkarvishal54@gmail.com)

---

*Open to Junior Developer, Full Stack, and Java Backend roles in Maharashtra and remote positions.*
